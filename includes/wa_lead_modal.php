...
.wl-dialog {
  ...
  background: var(--surf-1, #14181f); border: var(--glass-brd, 1px solid var(--line, #2a2f3a));
  border-radius: 24px; padding: 1.6rem 1.6rem 1.35rem;
  box-shadow: 0 40px 110px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.12);
  animation: wlPop .32s cubic-bezier(.22,1,.36,1);
}
[data-theme="light"] .wl-dialog { background: #fdfdfb; }
...
.wl-title { font-family: var(--font-display); font-size: 1.3rem; font-weight: 400; letter-spacing: .01em; color: var(--charcoal, #f3f3f0); margin: 0 0 .2rem; }
...
.wl-input {
  ...
  color: var(--charcoal, #f3f3f0); font: .9rem var(--font-body); transition: border-color .2s, box-shadow .2s;
}
...
.wl-btn {
  ...
  font: 800 .92rem var(--font-body); cursor: pointer; display: flex; align-items: center; justify-content: center; gap: .5rem;
  box-shadow: 0 10px 28px rgba(37,211,102,.32); transition: transform .3s cubic-bezier(.22,1,.36,1), box-shadow .3s, opacity .2s;
}
.wl-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 16px 38px rgba(37,211,102,.45); }
